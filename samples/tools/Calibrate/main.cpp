...
public:
    ...
    void copy_ir16_to_mat(astra::infraredframe_16& irFrame, Mat& mat)
    {
        int width = irFrame.resolutionX();
        int height = irFrame.resolutionY();
        ...
        const uint16_t* ir_data = irFrame.data();
        for (int y = 0; y < height; ++y, ir_data += width) {
            uint16_t* row_data = mat.ptr<uint16_t>(y);
            memcpy(row_data, ir_data, width * sizeof(uint16_t));
        }
    }
...
int main(int argc, char** argv)
{
    ...
    sf::RenderWindow window(windowed_mode, "Calibrate");
            ...
            case sf::Event::KeyPressed:
                {
                    ...
                    case sf::Keyboard::F:
                        ...
                        {
                            is_fullscreen = false;
                            window.create(windowed_mode, "Calibrate", sf::Style::Default);
                        }
                        else
                        {
                            is_fullscreen = true;
                            window.create(fullscreen_mode, "Calibrate", fullscreenStyle);
                        }
                        break;
                    case sf::Keyboard::R:
